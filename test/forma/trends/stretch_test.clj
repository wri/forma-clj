...
(tabular
 (fact
   "A monthly timeseries of identical values Jan-Dec, followed by one
additional, different value for the following January, should produce
a 16-day timeseries of 23 periods of identical values for the year,
plus one period, with a value from the 2nd January.

Note that this one additional period only covers the first half of
January. The ts value for the second period in Janaury cannot be
expanded until you've got data for February, since the second 16-day
period runs through Feb. 1 (Jan. 17 + 16 = Feb. 1)

We'll add a few additional values to make sure extending beyond
January works.

Expected outcome for adding twos to the end of the timeseries in a new
year:
...
This highlights that the expansion works for 32- to 16-day resolution
...
a year's worth of 16-day periods (23). Here we simplify by using the
same value for all periods, so `?periods-target-res` is just the
number of periods we should see, ignoring the value for those periods.

The correct number of periods for 16- and 8-day timeseries is given by
the number of directories in
ftp://e4ftl01p.cr.usgs.gov/MOLT/MOD13A1.005 for the range of interest.

Period 360 is 2000-01-01 at monthly resolution
Period 736 is 2000-01-01 at 16-day resolution

The problem at present: The output timeseries seems to be too short by
1-2 periods. So even if we have monthly data for December (starting on
12/1/2011), the output will only run through 11/16/2011 because the
16-day periods are bounded by 12/1/2011 rather than 12/31/2011."
...
(tabular
 (future-fact
  "Double-check that a year-long timeseries extended by an extra few
months will still have the correct. The values are all 1 for the first
year, then 2 for the 'extra' periods"
  (let [series (concat (repeat 12 1) (repeat ?extra-32 2))
        ts (schema/timeseries-value 360 series)
