...
(fact
  ...
  16-day dataset."
  (expand-to-days 384 [1 2 3] 384 "32" 0)
  => (concat (repeat 31 1) (repeat 28 2) (repeat 31 3)))
...
(fact
  "Expect to see values repeated the correct number of days given a
monthly dataset (e.g. 31 for January, 28 for non-leap year February."
  (expand-to-days 736 [1 2 3] 736 "16" 0)
  => (concat (repeat 16 1) (repeat 16 2) (repeat 16 3)))
...
focusing here on ensuring that the number of periods is correct, and
that the end of the year is accounted for correctly. Sixteen- and
8-day timeseries do not fit evenly into a year, so the final period of
each year will always be shorter than a normal period by several days.

Here, our timeseries values are repeated in a given year, as described
in `mk-ts`.
