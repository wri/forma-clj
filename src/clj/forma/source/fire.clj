;; This namespace has functions and Cascalog queries for processing
;; fire data.

;; Fires are recorded daily by the Fire Information For Resource
;; Management System (http://maps.geog.umd.edu) using the NASA Terra
;; and Aqua satellites.  They are described using the following
;; attributes which are stored in a CSV file:

;;   http://firefly.geog.umd.edu/firms/faq.htm#attributes

;; Here's an example CSV file, with header names shortened for this
;; description:

;; lat,lon,bright,scan,track,acq_date,acq_time,sat,conf,vers,bright_t31,frp
...
;; The FORMA algorithm depends on latitude, longitude, brightness,
;; confidence and acq_date. Before these fires can be used, they have
;; to fist get converted from latitude and longitude into MODIS pixel
;; coordinates at 500 meter resolution.

;; Once converted, we store each fire in a DataChunk Thrift object and
;; store it into a Pail. Pails are cool because you can append to
;; them. This allows us to incrementally process fire data. To see the
;; Thrift object definitions, check out the dev/forma.thirft IDL.

;; After getting converted, here's what a fire DataChunk looks like
;; for a single fire.
...
;; The FireTuple Thrift object defines properties of a single fire or
;; multiple fires. In this step, it represents a single fire. The
;; 'temp330' is set to 1 if the fire is greater than 330 degrees
;; Kelvin. The 'conf50' is set to 1 if the fire confidence is greater
;; than 50. If 'temp330' and 'conf50' are both 1, the 'bothPreds' is
;; set to 1. For a single fire, the 'count' is set to 1.

;; At the end of this step, we have a single Pail full of DataChunk
;; objects, one per fire, each which represents the fire in MODIS
;; pixel coordinates.

;; Note: We received fire data organized into files by month through
;; February 2010 from the University of Maryland. These are stored on
;; S3 in the s3://modisfiles/MonthlyFires directory. More recent fire
;; data are organized into daily files. The difference between them is
;; the date formatting and we have functions here to convert between
;; them.
