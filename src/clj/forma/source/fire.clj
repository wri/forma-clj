...
;; Question: This is for a single fire?
(def get-fire-tuple
  ...
  Example usage:
    ...
    > (let [src [[60 380]]                         
            ...
            [?tuple]
            (src ?conf ?kelvin)
            (get-fire-tuple ?conf ?kelvin :> ?tuple)))
                    
    ([#<FireTuple FireTuple(temp330:1, conf50:1, bothPreds:1, count:1)>])"
  (<- [?conf ?kelvin :> ?tuple]
      ...
      (schema/thrifter FireTuple ?temp-330 ?conf-50 ?both-preds ?count :> ?tuple)))

;; Question: Why not combine this predicate macro with get-fire-tuple?
(def fire-pred
  "Returns a Cascalog predicate macro that creates a FireTuple Thrift object.
  ...
  Output variables:
    ...
    ?fire-tuple - The FireTuple Thrift object representing this fire.
  ...
  Example usage:
    > (let [src [[\"1\" \"2\" \"331\" \"51\"]]]
           (??<-
            ...
            (fire-pred ?s-lat ?s-lon ?s-kelvin ?s-conf :> ?dataset ?t-res ?lat ?lon ?fire-tuple)))
    ([\"fire\" \"01\" 1.0 2.0 #<FireTuple FireTuple(temp330:1, conf50:1, bothPreds:1, count:1)>])"  
  (<- [?s-lat ?s-lon ?s-kelvin ?s-conf :> ?dataset ?t-res ?lat ?lon ?fire-tuple]
      (utils/strings->floats ?s-lat ?s-lon ?s-kelvin ?s-conf :> ?lat ?lon ?kelvin ?conf)
      (p/add-fields "fire" "01" :> ?dataset ?t-res)
      (get-fire-tuple ?conf ?kelvin :> ?fire-tuple)))
...
(defn fire-source-monthly
  ...
  (<- [?dataset ?date ?t-res ?lat ?lon ?tuple]
      ...
      (monthly->standard ?datestring :> ?date)
...
(defn fire-source-daily
  ...
  Output variables:
    ...
    ?lat - The fire latitude as a float.
    ?lon - The fire longitude as a float.
    ?fire-tuple - The FireTuple Thrift object representing the fire."
  [src]
  (<- [?dataset ?date ?t-res ?lat ?lon ?fire-tuple]
      (src ?line)
      (p/mangle [#","] ?line
                :> ?s-lat ?s-lon ?s-kelvin _ _ ?datestring _ _ ?s-conf _ _ _)
      (daily->standard ?datestring :> ?date)
      (fire-pred ?s-lat ?s-lon ?s-kelvin ?s-conf :> ?dataset ?t-res ?lat ?lon ?fire-tuple)
      (:distinct false)))
...
(defn reproject-fires
  ...
  [s-res src]
    (<- [?datachunk]
        ...
        (src ?dataset ?date ?t-res ?lat ?lon ?fire-tuple)
        (r/latlon->modis ?s-res ?lat ?lon :> ?h ?v ?s ?l)
        (schema/thrifter LocationProperty ModisPixelLocation ?s-res ?h ?v ?s ?l :> ?loc)
        (schema/thrifter DataChunk ?dataset ?loc ?fire-tuple ?t-res ?date :> ?datachunk)
        (:distinct false)))