...
(defn first-seq
  "second and penultimate rows of the H-P filter F matrix."
  [T]
  (let [s (concat [1 -2 1] (repeat (- T 3) 0))]
    [s (reverse s)]))

(defn second-seq
  "second and penultimate rows of the H-P filter F matrix."
  [T]
  (let [s (concat [-2 5 -4 1] (repeat (- T 4) 0))]
    [s (reverse s)]))

(defn third-seq
  "create a sequence of row vectors for the third order condition
  to be used in the specification of the Hodrick-Prescot filter.
  The parameter [T] "
  [T]
  (let [T-5 (- T 5)]
    (for [x (range (inc T-5))]
      (concat (repeat x 0) [1 -4 6 -4 1] (repeat (- T-5 x) 0)))))
...
(defn hp-F-matrix
  [T]
  (let [inner (insert-at 1 (second-seq T) (third-seq T))]
    (i/matrix (insert-at 1 (first-seq T) inner))))

(defn hp-filter
  [ts lambda]
  (let [T (count ts)
        smooth (i/solve
                (i/plus (i/mult lambda (hp-F-matrix T)) (identity-matrix T)))]
    (i/mmult smooth ts)))

(def plot1 (c/time-series-plot (range (count ndvi)) (hp-filter ndvi 20)))
(c/add-lines plot1 (range (count ndvi)) ndvi)
(c/add-lines plot1 (range (count ndvi)) (deseasonalize (hp-filter ndvi 20)))
