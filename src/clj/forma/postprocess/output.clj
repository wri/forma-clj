...
(defn clean-probs
  "Accepts a timeseries of probabilities, and returns a 'cleaned' time
  series.  Specifically, the cleaned timeseries Smooth the
  probabilities with a backward-looking moving average, make the
  series monotonically increasing, and finally make each probability
  0->1 an integer 0->100"
  [ts]
  (let [window 3]
    [(vec (->> (backward-looking-mavg window ts)
               (reductions max)
               (map #(round (* % 100)))))]))
